# Proyecto-Python-y-Power-BI-Encuesta-Satisfaccion-de-Clientes

He empleado Python para el tratamiento de datos que finalmente se conectarán a Power BI para un Dashboard.

---

# Proyecto Python y Power BI: Encuesta Satisfacción Clientes

**Autor del Proyecto:**

* **Github_Autor:** [OscarOspinaB](https://github.com)
* **Linkedin :** [Perfil Linkedin](https://linkedin.com)

---

### 🚀 Mínimo Producto Viable

El objetivo de que los datos hayan sido limpiados para poder crear un Dashboard en Power BI ha sido cumplido. No obstante, el ejercicio está sujeto a mejora continua para obtener información de valor.

El notebook se encuentra subido en este mismo repositorio donde se evidencia el proceso de tratamiento de los datos y pueden acceder al dashboard interactivo de Power BI a través del siguiente enlace: 

🔗 [Dashboard Power BI: Encuesta Satisfacción Clientes](https://app.powerbi.com/view?r=eyJrIjoiMjc1M2U1NDAtNzFiYS00OWFmLWI0ZDAtODZiMGI5OWNhOTdhIiwidCI6Ijk5ZTFlNzIxLTcxODQtNDk4ZS04YWZmLWIyYWQ0ZTUzYzFjMiIsImMiOjR9)

---
## Vistazo al Dashboard

<img width="1431" height="752" alt="image" src="https://github.com/user-attachments/assets/267a9b00-e980-4791-86d7-790a26c23c9f" />



<img width="1438" height="758" alt="image" src="https://github.com/user-attachments/assets/25b89dd4-5445-440f-af59-5b15658e69f2" />



---
## 📊 Fuente de los datos
---

Los datos provienen de Kaggle, de la usuaria: `Data Regina`, pueden acceder al dataset desde el siguiente link:

[Datasets para Proyecto BI - Satisfacción Clientes](https://kaggle.com)

<img width="1150" height="579" alt="image" src="https://github.com/user-attachments/assets/89a440c8-fb56-4d16-ac90-2002f4f70077" />

---




---
### ⚖️ Licencia
Este proyecto está bajo la Licencia **MIT**.




